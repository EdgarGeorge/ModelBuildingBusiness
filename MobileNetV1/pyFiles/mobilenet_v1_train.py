...
flags.DEFINE_float('width_multiplier', 1.0, 'Width multiplier for mobilenet')
flags.DEFINE_bool('quantize', False, 'Quantize training')
flags.DEFINE_string('fine_tune_checkpoint', '',
                    'Checkpoint from which to start finetuning.')
flags.DEFINE_string('checkpoint_dir', 'checkpoint',
                    'Directory for writing training checkpoints and logs')
flags.DEFINE_string('dataset_dir', 'dataset//flowers', 'Location of dataset')
...
def flower_input(is_training):
  """Data reader for flowers.
  ...
  """
  if is_training:
    dataset = dataset_factory.get_dataset('flowers', 'train',
                                          FLAGS.dataset_dir)
  else:
    dataset = dataset_factory.get_dataset('flowers', 'validation',
                                          FLAGS.dataset_dir)
...
def build_model():
  ...
  with g.as_default(), tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
    inputs, labels = flower_input(is_training=True)
    #with slim.arg_scope(V1Demo.V1Demo_arg_scope(is_training=True)):
    logits, _ = V1Demo.mobilenet(
        inputs,
